...
const { repairCodexWindowsSandbox } = require("./codex-sandbox-repair.cjs");
...
class RuntimeHost {
  ...
  async setupCore() {
    ...
    if (this.platform === "win32") {
      const sandboxRepair = repairCodexWindowsSandbox({
        codexHome: this.codexHome,
        smoke: true,
      });
      this.logger.info("codex.windows_sandbox_ready", {
        version: sandboxRepair.version,
        releaseName: sandboxRepair.releaseName,
        changed: sandboxRepair.changed,
        smokeTested: sandboxRepair.smokeTested,
        smokeOk: sandboxRepair.smokeOk,
      });
    }
  ...
  async upgradeManagedRuntime() {
    ...
    if (this.platform === "win32") {
      try {
        ...
        this.logger.info("codex.windows_sandbox_startup_check", {
          version: sandboxRepair.version,
          releaseName: sandboxRepair.releaseName,
          changed: sandboxRepair.changed,
          smokeTested: sandboxRepair.smokeTested,
        ...
        this.logger.warn("codex.windows_sandbox_startup_check_failed", {
          message: error instanceof Error ? error.message : String(error),
        });
      }
    }
