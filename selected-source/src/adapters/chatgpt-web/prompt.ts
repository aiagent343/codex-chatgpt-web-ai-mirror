...
export function compileChatGptWebPrompt(
  ...
  capabilities: ChatGptWebCapabilities,
  turnToken?: string,
  nativeToolRelay = false,
): CompiledChatGptWebPrompt {
  const mode = resolveChatGptWebModelMode(parsed.modelId, parsed.options.reasoning, capabilities);
  const relayEnabled = nativeToolRelay
    && !parsed._compactionRequest
    && !mode.localTools
    && (parsed.context.tools?.length ?? 0) > 0;
  if (nativeToolRelay && mode.localTools) {
    throw new Error("Native Responses tool relay must not be combined with the Codex Native MCP bridge");
  }
  ...
  const envelope = {
    ...
    system,
    messages,
    ...(relayTools ? { tools: relayTools } : {}),
    ...(relayEnabled && parsed.options.toolChoice !== undefined ? { tool_choice: parsed.options.toolChoice } : {}),
    ...(relayEnabled && parsed.options.parallelToolCalls !== undefined
  ...
  const transportContract = parsed._compactionRequest
    ...
    : relayEnabled
    ? [
      "Local Codex tools are executed by the outer Codex harness through the native Responses function_call protocol. They are NOT attached as ChatGPT plugins or MCP connectors.",
      "The complete list of tools available in this exact Codex round is in codex_context_json.tools. Never invent, rename, or call a tool that is not listed there.",
      "If the task needs fresh local inspection, a command, a file mutation, image inspection, user interaction, or another listed Codex tool, return a tool relay decision instead of claiming the action happened.",
      'For one tool call return exactly: {"decision":"tool_call","name":"EXACT_TOOL_NAME","arguments":{...}}',
      'For parallel calls return exactly: {"decision":"tool_calls","calls":[{"name":"EXACT_TOOL_NAME","arguments":{...}}]}',
      'For a freeform tool whose tool entry has "freeform":true, put its raw payload in arguments.input as a string.',
      'When no Codex tool is needed, return exactly: {"decision":"final","content":"THE COMPLETE USER-FACING ANSWER"}',
      "Do not wrap the relay JSON in Markdown fences. Do not emit commentary before or after it.",
      "A tool call ends this browser response. Codex will execute it locally and the next browser request will contain the matching tool_result; continue from that real result instead of assuming success.",
      "Respect tool_choice and parallel_tool_calls from codex_context_json whenever present.",
    ]
    : mode.localTools
    ? [
  ...
  const transportResume = parsed._compactionRequest
    ...
    ? [
      "<codex_transport_resume>",
      "The task context is complete. Decide whether the latest active request needs one of codex_context_json.tools.",
      "Return exactly one relay JSON object now. If a prior tool_result is present, use its real result to decide the next tool call or the final answer.",
      "</codex_transport_resume>",
    ]
    : mode.localTools
    ? [
  ...
  const text = [
    ...
    ...transportContract,
    relayEnabled
      ? "Return only the native Responses relay JSON object described above."
