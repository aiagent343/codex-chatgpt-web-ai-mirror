...
import { parseNativeToolDecision, type NativeToolDecision } from "./native-tool-decision";
...
interface NativeRelayRunEntry {
  promise: Promise<NativeToolDecision>;
  settledAt?: number;
}

const NATIVE_RELAY_REPLAY_TTL_MS =
  10 * 60_000;

const nativeRelayRuns =
  new Map<string, NativeRelayRunEntry>();

function pruneNativeRelayRuns(now = Date.now()): void {
  for (const [key, entry] of nativeRelayRuns) {
    if (
      entry.settledAt !== undefined &&
      now - entry.settledAt >= NATIVE_RELAY_REPLAY_TTL_MS
    ) {
      nativeRelayRuns.delete(key);
    }
  }
}
...
export function createChatGptWebAdapter(provider: CodexProviderConfig): ProviderAdapter {
  ...
  return {
    ...
    async runTurn(parsed, incoming, emit) {
      ...
      const mode = resolveChatGptWebModelMode(parsed.modelId, parsed.options.reasoning, turnCapabilities);
      const nativeToolRelay = !parsed._compactionRequest
        && !mode.localTools
        && (parsed.context.tools?.length ?? 0) > 0;
      if (nativeToolRelay) {
        /*
         * Native Responses retries must be idempotent.
         *
         * A Codex/SSE reconnect is a retry of the SAME logical Responses
         * request, not permission to submit the prompt to ChatGPT again.
         *
         * Use the same stable execution identity already used by the normal
         * ChatGPT turn-session path.
         */
        pruneNativeRelayRuns();

        const executionKey =
          `${executionNamespace}:native:${chatGptTurnExecutionKey(parsed)}`;
        ...
        let entry =
          nativeRelayRuns.get(executionKey);

        if (!entry) {
          console.info(
            `[chatgpt-web] native Responses tool relay start`
            + ` trace=${traceId}`
            + ` tools=${parsed.context.tools?.length ?? 0}`,
          );

          /*
           * IMPORTANT:
           *
           * Do NOT bind the underlying browser turn to this HTTP/SSE
           * connection's abort signal.
           *
           * If Codex disconnects and reconnects, the logical browser turn
           * must continue running so the retry can attach to the same
           * Promise instead of creating another Temporary Chat.
           */
          const promise =
            worker.run({
              traceId,
          ...
          void promise.then(
            () => {
              if (
                nativeRelayRuns.get(executionKey) === createdEntry
              ) {
                createdEntry.settledAt =
                  Date.now();
              }
            },
            error => {
              ...
              /*
               * A retryable upstream browser failure is allowed to create a
               * genuinely fresh Temporary Chat on the next Codex attempt.
               *
               * Parser/protocol errors are intentionally retained briefly:
               * retries must replay the same failure rather than sending the
               * same huge prompt to ChatGPT five more times.
               */
              if (
                error instanceof ChatGptWebAdapterError &&
                error.retryable
              ) {
                nativeRelayRuns.delete(
                  executionKey,
                );
              }
              else {
          ...
          console.info(
            `[chatgpt-web] native Responses tool relay replay`
            + ` trace=${traceId}`
            + ` settled=${entry.settledAt !== undefined}`,
          );
        }

        /*
         * Each transport connection owns only its wait/heartbeat lifecycle.
         * Aborting this connection must not abort the shared browser turn.
         */
        emit({
          type: "heartbeat",
        });
        ...
        let decision: NativeToolDecision;
        ...
        emitToolBatch(
          ...
          estimateChatGptWebUsage(
            ...
            {
              reasoning: [],
              toolRequests: decision.requests,
            },
            turnCapabilities,
          ),
          emit,
